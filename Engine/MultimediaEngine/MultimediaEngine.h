...
protected:
	...
	// Parameters: pGraph for the graph to remove filters from.
...
public:
	...
	// Parameters: wsInputFilePath - The file path for the multimedia file.
	...
	// Parameters: sInputFilePath - The file path for the audio file.
	// Return values: TRUE or 1 for success, FALSE or 0 for fail.
	//
	bool BASSAudioPlayer(std::string sInputFilePath);

	// MMAPIAudioPlayer - Plays a WAV file using the Windows MultiMedia API.
	// Parameters: wsInputFilePath - The file path for the WAV file.
	// Return values: TRUE or 1 for success, FALSE or 0 for fail.
	//
	...
	// Parameters: wsText - Text to be outputted as Text-To-Speech.
	// Return values: TRUE or 1 for success, FALSE or 0 for fail.
	//
	bool TTSOutput(std::wstring wsText);

	// BeepSound - Produces access to produce a low-latency, audible and constant beep sound.
	// Parameters: dFrequency - Set the frequency of the beep sound. This is measured in Hertz.
	//            dDuration - Set the duration of the sound. This is measured in MILLISECONDS.
	//            bExitOnESCKeyPress - Exit on ESC keypress while sound is being outputted.
	// Return values: TRUE or 1 for uninterrupted output, and FALSE or 0 for ESC key pressed.
	// Notes: Providing no arguments will give a 500hz beep sound for 1 second.
	//
	bool BeepSound(long double dFrequency = 500, long double dDuration = 1, bool bExitOnESCKeyPress = false);
};